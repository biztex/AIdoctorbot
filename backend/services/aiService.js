...
class AIService {
  static async diagnoseSymptoms(symptoms, chatHistory = []) {
    const systemPrompt = `
...
上記症状から考えられる病名と緊急レベルを診断してください。
    ...
    try {
      ...
      const mockResponse = {
        ...
        summary:
          text ||
          "インフルエンザの可能性が高いです。水分補給と安静を心がけ、症状が悪化する場合は医療機関を受診してください。",
...
ただし、最終的な診断は医師の診察が必要であることを必ず伝えてください。
    ...
    try {
      ...
      return text || "ご質問にお答えできませんが、症状が続く場合は医療機関を受診することをお勧めします。"
    } catch (error) {
